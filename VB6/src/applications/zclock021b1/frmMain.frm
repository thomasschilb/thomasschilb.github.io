...
' zClock v0.2.1
'
' 0.1          - startet simple digital clock within a label
' 0.1.1        - redesigned form to since courier theme black&white
' 0.2.1 BETA 1 - added 3d stars to the hole form,
'                but therefore are graphic faults,
'                so i have to redraw all in the loop?!? flickering?!?
'
...
If Z(i) < -255 Then Z(i) = 255
tmpZ(i) = Z(i) + Zoom
tmpX(i) = (X(i) * K / tmpZ(i)) + (frmMain.Width / 2)
tmpY(i) = (Y(i) * K / tmpZ(i)) + (frmMain.Height / 2)
Radius = 1
StarColor = 256 - Z(i)
